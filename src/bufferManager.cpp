...
/**
 * @brief The buffer manager is also responsible for writing pages. This is
 * called when new matrices are created/loaded.
 *
 * @param matrixName 
 * @param pageRowVal 
 * @param pageColVal 
 * @param pageData
 */
void BufferManager::writeMatrix_Page(string matrixName, int rowVal, int colval, vector<vector<int>> data)
{
    logger.log("BufferManager::writeMatrixPage (in bufferManager.cpp)");
    Page page(matrixName, rowVal, colval, data, matrixDims, matrixDims);
    page.writeMatrixPage();
}


vector<vector<int>> BufferManager::readMatrixPage(string matrixPage){
    fstream fin(matrixPage, ios::in);
    vector<vector<int>> matrix(matrixDims, vector<int>(matrixDims,0));
    int val;
...
Page BufferManager::insertMatrixIntoPool(string matrix, string matrixPage, int rowId, int colId, vector<vector<int>>& vec, int rows, int cols){
    logger.log("BufferManager::insertMatrixIntoPool");
    Page page(matrixPage, rowId, colId, vec, rows, cols);
    if(this->pages.size()>=BLOCK_COUNT) pages.pop_front();
    pages.push_back(page);
    return page;
}

//getMatrix function implementation.....
Page BufferManager::getMatrix(string matrix, int rowId, int colId){
    logger.log("BufferManager::getMatrix");
    string matrixPage = "../data/temp/" + matrix + "_matrix_page_" + to_string(rowId) + "_" + to_string(colId);
    if(this->inPool(matrixPage)){
        return this->getFromPool(matrixPage);
    }
    else{
        vector<vector<int>> vec = this->readMatrixPage(matrixPage);
        // if(rowId==0 && colId==0){
        // for(int i=0;i<matrixDims;i++){
        ...
        return this->insertMatrixIntoPool(matrix, matrixPage, rowId, colId, vec, matrixDims, matrixDims);
    }
}


void BufferManager::clearMainMemory(){
